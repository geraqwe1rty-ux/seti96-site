...
test('only confirmed delivery succeeds; client relay flag cannot spoof success',async()=>{const dir=await mkdtemp(path.join(tmpdir(),'seti96-lead-test-'));try{const handler=createUslugiLeadHandler({dataDir:dir,publicRelayUrl:'',deliverDirect:async()=>({ok:false}),env:{}});const res=response();await handler({hostname:'uslugi.seti96.ru',body:{...input,clientRelayDelivered:true}},res,()=>{});assert.equal(res.code,502);assert.equal(res.data.saved,true);const rows=await readUslugiLeads(dir);assert.equal(rows.length,1);assert.equal(rows[0].telegram_status,'доставка не настроена')}finally{await cleanup(dir)}});
test('parallel requests preserve all records and delivery status',async()=>{const dir=await mkdtemp(path.join(tmpdir(),'seti96-lead-test-'));try{let delivered=0;const handler=createUslugiLeadHandler({dataDir:dir,publicRelayUrl:'',deliverDirect:async()=>{delivered++;return{ok:true}},env:{TELEGRAM_BOT_TOKEN:'test-only',TELEGRAM_CHAT_ID:'test-only'}});const responses=await Promise.all(Array.from({length:12},async(_,i)=>{const res=response();await handler({hostname:'uslugi.seti96.ru',body:{...input,phone:'+7999000'+String(i).padStart(4,'0')}},res,()=>{});return res}));assert.ok(responses.every(x=>x.code===200&&x.data.ok));const rows=await readUslugiLeads(dir);assert.equal(rows.length,12);assert.equal(new Set(rows.map(x=>x.id)).size,12);assert.equal(delivered,12);assert.ok(rows.every(x=>x.telegram_status==='доставлено (напрямую)'))}finally{await cleanup(dir)}});
